	...
	, mRange(100.f)
{
	...
	GetAnimator()->CreateAnimation(L"IceBolt", pTex, Vect2(0.f, 0.f), Vect2(63.f, 32.f), Vect2(63.f, 0.f), 0.1f, 5);
	...
	if (pObj->GetName() == L"Monster")
	{
		...
		for (size_t i = 0; i < vecMon.size(); ++i)
		{
			...
			if (mRange > (monsterPos - pObj->GetPos()).Length())		// 일정 범위안에 있는 몬스터들을 얼음 상태로 만듦
			{
				...
				pIce->SetPos(vecMon[i]->GetPos());
				pIce->SetAlpha(200);
				pIce->AddIceMonster(vecMon[i]);							// 얼어있는 몬스터를 벡터로 받아놓음
